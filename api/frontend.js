...
export default async function handler(req, res) {
  ...
  try {
    ...
    // IMPORTANT:
    // Do not rewrite the application's JavaScript here. Earlier runtime
    // source-repair/replacement logic could mutate the canonical HTML and
    // produce browser syntax errors such as "Unexpected end of input".
    // The frontend file is now served as-is, with only a small parser helper
    // inserted before the application's own scripts.
    if (!html.includes('window.parsePlate = function')) {
    ...
    res.setHeader('X-Avtodrom-Frontend', 'canonical-clean-v5');
    return res.end(html);
  } catch (error) {
