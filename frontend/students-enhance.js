...
/*
  MUHIM: Excel tarixi avtomobilga bog‘liq emas.
  Eski index.html handlerida plate tekshiruvi qolgan bo‘lsa ham,
  quyidagi document-level CAPTURE handler uni oldindan to‘xtatadi.
*/
...
function isExcelHistoryButton(el){
  if(!el)return false;
  const btn=el.closest?.('button');
  if(!btn)return false;
  const text=(btn.textContent||'').replace(/\s+/g,' ').trim().toLowerCase();
  return text.includes('excel') && text.includes('tarix');
}

function getStudentFromCard(card){
  if(!card)return null;
  const index=[...document.querySelectorAll('#studentList > *')].indexOf(card);
  const cache=window.__avtodromStudentsCache;
  return Array.isArray(cache)&&index>=0 ? cache[index] : null;
}

/* Eng yuqori darajadagi Excel handler: eski plate talab qiladigan handlerlardan oldin ishlaydi. */
if(!window.__studentExcelGlobalFix){
  window.__studentExcelGlobalFix=true;
  document.addEventListener('click',function(e){
    const target=e.target;
    if(!isExcelHistoryButton(target))return;
    const btn=target.closest('button');
    const card=btn.closest('#studentList > *')||btn.closest('.student-card-enhanced')||btn.parentElement?.parentElement;
    const student=getStudentFromCard(card);
    if(!student)return;
    e.preventDefault();
...
function installExcel(card,student){
  ...
  card.addEventListener('click',e=>{
    if(!isExcelHistoryButton(e.target))return;
    e.preventDefault();
    e.stopPropagation();
