...
(function(){
  function safeFrozenFinish(id){
    ...
    box.innerHTML = '<div class="finish" style="margin-top:10px">'
      ...
      + '<div class="muted" style="margin-bottom:10px">'+escx(v.model || '')+(v.driver_name ? ' • '+escx(v.driver_name) : '')+(v.school_name ? ' • '+escx(v.school_name) : '')+(v.group_name ? ' • '+escx(v.group_name) : '')+(v.student_name ? ' • '+escx(v.student_name) : '')+'</div>'
      + (school
      ...
      + '<div class="actions"><button type="button" class="btn green" id="sfsubmit'+id+'">✓ Tugatish</button><button type="button" class="btn light" id="sfcancel'+id+'">Bekor</button></div><div id="sferr'+id+'" class="err"></div></div>';
    let method = 'cash';
    ...
    const updateSplit = () => { const c=Number(document.getElementById('sfc'+id)?.value||0), t=Number(document.getElementById('sft'+id)?.value||0), out=document.getElementById('sftot'+id); if(out) out.textContent=moneyx(c+t); };
    ...
    document.getElementById('sfsubmit'+id)?.addEventListener('click', async () => {
      const err=document.getElementById('sferr'+id); if(err) err.textContent='';
      const amount=school?0:Number(document.getElementById('sfa'+id)?.value||0);
      if(!school && (!Number.isFinite(amount)||amount<=0)){if(err) err.textContent='Yakuniy summani kiriting';return;}
      let cash=0,terminal=0;
      if(!school){
        if(method==='cash') cash=amount;
        else if(method==='terminal') terminal=amount;
        else { cash=Number(document.getElementById('sfc'+id)?.value||0); terminal=Number(document.getElementById('sft'+id)?.value||0); if(cash<0||terminal<0||Math.abs(cash+terminal-amount)>0.01){if(err) err.textContent='Naqd + terminal summasi umumiy summaga teng bo‘lishi kerak';return;} }
      }
      const submit=document.getElementById('sfsubmit'+id); if(submit){submit.disabled=true;submit.textContent='Saqlanmoqda...';}
      try{
        await api('/sessions/'+id+'/finish-frozen',{method:'POST',body:JSON.stringify({amount,paymentMethod:school?'cash':method,cashAmount:cash,terminalAmount:terminal})});
      ...
      }catch(e){if(err) err.textContent=e.message||'Muzlatilgan avtomobilni tugatishda xatolik';if(submit){submit.disabled=false;submit.textContent='✓ Tugatish';}}
    });
  }
  document.addEventListener('click',function(e){
    const btn=e.target&&e.target.closest?e.target.closest('button'):null;if(!btn)return;
    const frozenCard=btn.closest('.frozen');if(!frozenCard)return;
    if((btn.textContent||'').trim()!=='Tugatish')return;
    const holder=frozenCard.nextElementSibling;const id=holder&&holder.id&&holder.id.indexOf('ff')===0?holder.id.slice(2):null;if(!id)return;
    ...
    res.sendFile = async (filePath, options, callback) => {
      try {
        if (String(filePath).endsWith("/frontend/index.html")) {
          ...
          if (!res.headersSent) { res.statusCode = 200; res.setHeader("Content-Type", "text/html; charset=utf-8"); }
