...
async function reports(req,res,kind){
  ...
  if(kind==='active'||kind==='frozen'){
    const status=kind==='active'?'active':'frozen'; const r=await pool.query(`SELECT se.id,v.plate,v.model,v.driver_name,se.started_at,se.finished_at,se.duration_seconds,se.school_id,se.group_id,se.student_id,ds.name school_name,g.name group_name,st.full_name student_name FROM sessions se LEFT JOIN vehicles v ON v.id=se.vehicle_id LEFT JOIN driving_schools ds ON ds.id=se.school_id LEFT JOIN school_groups g ON g.id=se.group_id LEFT JOIN students st ON st.id=se.student_id WHERE se.status=$1 ORDER BY se.started_at DESC`,[status]);return send(res,200,r.rows);
  }
