...
async function frozenFinishSubmit(id){
  const v=(typeof frozen!=='undefined'?frozen:[]).find(x=>String(x.id)===String(id));
  if(!v)return;
  const err=document.getElementById('ferr'+id),school=!!v.student_id,amount=school?0:Number(document.getElementById('fa'+id)?.value||0),method=school?'cash':((typeof pay!=='undefined'&&pay[id])||'cash');
